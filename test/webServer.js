...
if (process.argv.length < 2) {
        console.log(
            "Usage: node webServer.js [web-root] [http-port]"
        );
        process.exit();
}

const WEB_ROOT = process.argv[2] || "./";
const HTTP_PORT = process.argv[3] || 8086;
...
var funWebSvr = function (req, res) {
    ...
    //使用路径解析模块,组装实际文件路径 
    var filePath = libPath.join(WEB_ROOT, pathName);
...
//开始侦听指定端口
webSvr.listen(HTTP_PORT, function () {
    //向控制台输出服务启动的信息 
    console.log('WebServer running at http://localhost:'+HTTP_PORT);
